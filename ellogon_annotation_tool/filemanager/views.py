    ...
    def put(self, request):
        try:
            ...
            project = self.queryset.get(
                name=request.data["current_name"], owner=owner)
        except Exception as e:
    ...
    def post(self, request):
        try:
            ...
            project = Project.objects.filter(
                name=request.data["name"], owner=owner)
...
class CollectionCreate(APIView):
    def post(self, request, format='json'):
        ...
        try:
            ...
            data["owner"] = owner.pk
            project = Project.objects.filter(
                name=request.data["project"], owner=owner)
...
class CollectionUpdate(GenericAPIView, UpdateModelMixin):
    ...
    def put(self, request):
        try:
            ...
            print(owner)
            project = (Project.objects.filter(
                name=request.data["project"], owner=owner)).get()
            print(project)
            collection = self.queryset.get(
                name=request.data["current_name"], owner=owner, project=project)
            print(collection)
        except Exception as e:
            print(e)
        ...
        if not data:
            ...
            data["updated"] = datetime.now()
            serializer = self.serializer_class(
                collection, data=data, partial=True)
            if serializer.is_valid():
                collection = serializer.save()
                if collection:
                    #json = serializer.data
    ...
    def post(self, request):
        try:
            ...
            collection = Collection.objects.filter(
                name=request.data["name"], owner=owner, project=project)
...
class DocumentOpen(APIView):
    ...
    def get(self, request):
        ...
        try:
            owner = CustomUser.objects.get(email=data["owner"])
            project = (Project.objects.filter(
                name=data["project"], owner=owner)).get()
            collection = (Collection.objects.filter(
                name=data["collection"], owner=owner, project=project)).get()
            document = self.queryset.get(
                name=data["name"], collection=collection, owner=owner, project=project)
            document_details = {
                "name": data["name"], "collection": data["collection"], "project": data["project"], "text": document.text}
...
class DocumentUpload(APIView):
    def post(self, request, format='json'):
        ...
        try:
            ...
            project = (Project.objects.filter(
                name=data[0]["project"], owner=owner)).get()
            collection = Collection.objects.filter(
                name=request.data[0]["collection"], owner=owner, project=project)
        except Exception as e:
            print(e)
            return Response(data={"create": False}, status=status.HTTP_400_BAD_REQUEST)
        try:
    ...
    def put(self, request):
        ...
        try:
            ...
            project = (Project.objects.filter(
                name=request.data["project"], owner=owner)).get()
            collection = (Collection.objects.filter(
                name=request.data["collection"], owner=owner, project=project)).get()
            document = self.queryset.get(
                name=request.data["current_name"], collection=collection, owner=owner, project=project)
           # print(document)
        except Exception as e:
            # print(e)
            return Response(data={"updated": False}, status=status.HTTP_404_NOT_FOUND)
        ...
        for field in self.update_fields:
            if field in request.data.keys():
                if (field == "updated_by"):
                    data[field] = (CustomUser.objects.get(
                        email=request.data['email'])).pk
                    continue
                data[field] = request.data[field]
    ...
    def post(self, request):
        try:
            ...
            document = Document.objects.filter(
                name=request.data["name"], owner=owner, collection=collection, project=project)
    ...
    def get(self, request, email):
        try:
            ...
            for project in projects:
                record = {"id": "P"+str(project.pk), "name": project.name, "encoding": project.encoding, "created": project.created.strftime(
                    "%d/%m/%Y, %H:%M"), "updated": project.updated.strftime("%d/%m/%Y, %H:%M"), "parentName": None}
                userdata.append(record)

                collections = Collection.objects.filter(
                    owner=owner, project=project)
                for collection in collections:
                    record = {"id": "C"+str(collection.pk), "name": collection.name, "encoding": collection.encoding, "created": collection.created.strftime(
                        "%d/%m/%Y, %H:%M"), "updated": collection.updated.strftime("%d/%m/%Y, %H:%M"), "parentName": project.name}
                    userdata.append(record)
                    documents = Document.objects.filter(
                        owner=owner, project=project, collection=collection)
                    for document in documents:
                        record = {"id": "D"+str(document.pk), "name": document.name, "encoding": document.encoding, "created": document.created.strftime(
                            "%d/%m/%Y, %H:%M"), "updated": document.updated.strftime("%d/%m/%Y, %H:%M"), "parentName": collection.name, "rootName": project.name}
                        userdata.append(record)
...
class HandlerApply(APIView):
    ...
    def get(self, request, format=None):
        ...
        """
        handlers = [{"name": h.name, "type": h.function_name} for h in Handler.objects.all()]
