...
set GUI_Options(GUI_Theme) Moz
if {![info exists DoNotLoadImages]} {
  foreach file [glob $HomeDir/share/Themes/Moz/*.gif] {
    set image [file root [file tail $file]]
    puts "Reloading Image: $file"
    if {[catch {image create photo $image -file \
      [file join $HomeDir share Themes Moz $image.gif]}]} {
      GUI_LoadImage $image
    }
  }
  catch {image create photo IToolbar -file \
      [file join $HomeDir share Themes Moz IToolbar.gif]}
  catch {image create photo IToolbarVertical -file \
      [file join $HomeDir share Themes Moz IToolbarVertical.gif]}
...
source [file join $HomeDir share Themes Moz DrawModule.tcl]
if {[string equal $tcl_platform(platform) unix]} {
  source [file join $HomeDir share Themes Moz MessageDlg.tcl]
}
