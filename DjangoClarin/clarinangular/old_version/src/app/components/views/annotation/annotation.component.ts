        ...
        if (response.success && response.data.length > 0) {
          ...
          if (typeof (documentFound) != "undefined") {
            ...
            if (response.data.filter(doc => doc.document_id === documentFound.document_id).length == 1
              ...
              console.warn(" -> Erasing Temp annotations for annotator:", documentFound.annotator_type);
              this.tempAnnotationService.destroy(documentFound.collection_id, documentFound.document_id, documentFound.annotator_type)
                .then((response) => {
                  console.warn(" -> Closing Document for annotator:", documentFound.annotator_type);
                  this.openDocumentService.destroy(documentFound.document_id, documentFound.annotator_type);
                  this.createDocumentSelectionModal();
                }, (error) => {
                  this.dialog.open(ErrorDialogComponent, {
                    data: new ConfirmDialogData("Error", "Database error. Please refresh the page and try again.")
            ...
            } else if (documentFound.db_interactions == 0) {
              console.warn("Document opened by current user, no db_interactions have occurred and is shared:", response.data.filter(doc => doc.document_id === documentFound.document_id));
              console.warn(" -> Closing Document for annotator:", documentFound.annotator_type);
              this.openDocumentService.destroy(documentFound.document_id, documentFound.annotator_type);
            } else if (
